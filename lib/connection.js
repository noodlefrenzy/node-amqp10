...
Connection.prototype._nextChannel = function() {
  ...
  throw new errors.OverCapacityError('Out of available ' + this.connectionParams.channelMax + ' channels');
};
...
Connection.prototype._processOpenFrame = function(frame) {
  ...
  var valid = true;
  this.connectionParams.maxFrameSize =
    Math.min(this.connectionParams.maxFrameSize, frame.maxFrameSize || constants.defaultMaxFrameSize);
  this.connectionParams.channelMax =
    Math.min(this.connectionParams.channelMax, frame.channelMax || constants.defaultChannelMax);
  this.connectionParams.idleTimeout =
    Math.min(this.connectionParams.idleTimeout, frame.idleTimeout || constants.defaultIdleTimeout);
