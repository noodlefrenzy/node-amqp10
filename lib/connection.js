...
Connection.prototype._connect = function(address, sasl) {
    ...
    if (isSSL) {
        if (!self._sslOptions) {
            throw new exceptions.ArgumentError('sslOptions');
        }
        self._sslOptions.port = self.address.port;
        self._sslOptions.host = self.address.host;
        self.client = tls.connect(self._sslOptions);
