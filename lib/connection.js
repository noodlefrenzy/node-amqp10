...
var Connection = function (options) {
    ...
    this.connectionParams = {
        ...
        max_frame_size: options.maxFrameSize || constants.defaultMaxFrameSize,
        channel_max: options.channelMax || constants.defaultChannelMax,
        idle_time_out: options.idleTimeout || constants.defaultIdleTimeout,
        outgoing_locales: options.outgoingLocales || constants.defaultOutgoingLocales,
        incoming_locales: options.incomingLocales || constants.defaultIncomingLocales
    };
...
Connection.prototype.sendFrame = function(frame) {
    ...
    debug('Sending frame: ' + frameBuf.toString('hex'));
...
Connection.prototype._nextChannel = function() {
    ...
    throw new exceptions.OverCapacityError('Out of available channels');
};
...
Connection.prototype._tryReceiveHeader = function() {
    if (this._curbuf.length >= constants.amqp_version.length) {
...
Connection.prototype._tryReceiveFrame = function() {
    try {
        ...
        if (frame === undefined) {
            ...
            debug('Matched frame: ' + JSON.stringify(frame));
...
Connection.prototype._processOpenFrame = function(frame) {
    ...
    var valid = true;
    if (frame.maxFrameSize) this.connectionParams.maxFrameSize = Math.min(this.connectionParams.maxFrameSize, frame.maxFrameSize);
    if (frame.channelMax) this.connectionParams.channelMax = Math.min(this.connectionParams.channelMax, frame.channelMax);
    if (frame.idleTimeout) this.idleTimeout = Math.min(this.connectionParams.idleTimeout, frame.idleTimeout);
