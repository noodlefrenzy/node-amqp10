...
Connection.prototype._receiveAny = function() {
    var decoded = codec.decode(this.curbuf);
    if (decoded) {
        if (decoded instanceof DescribedType) {
            var frame = FrameReader.read(decoded);
            if (!frame) {
                console.warn('Failed to match frame for ' + JSON.stringify(decoded));
            } else {
                debug('Matched frame!');
                debug(JSON.stringify(frame));
            }
        ...
        debug('No matching decode for ' + this.curbuf.toString('hex'));
    }
};
