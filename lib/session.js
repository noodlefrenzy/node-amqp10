...
Session.prototype.sendMessage = function(link, message, options) {
    link.sendMessage(message, options);
};
                ...
                if (frame.handle !== null) {
                    ...
                    if (frame.linkCredit && frame.linkCredit > 0) {
                        if (this._senderLinks.length === 1) {
                            ...
                            console.warn('Unsure what to do with FLOW frame with no handle and link credit when more than one sender link exists.');
                        }
                    }
                }
...
Link.prototype.sendMessage = function(message, transferOptions) {
