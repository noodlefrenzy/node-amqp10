...
Session.prototype.sendMessage = function(link, message, options) {
    var transferFrame = new TransferFrame(options);
    transferFrame.channel = session.channel;
    transferFrame.handle = link.handle;
    transferFrame.message = message;
    this.connection.sendFrame(transferFrame);
};
...
Session.prototype._nextHandle = function() {
    ...
    throw new exceptions.OverCapacityError('Out of available handles (Max = ' + this.sessionParams.handleMax + ')');
};
            ...
            if (frame instanceof EndFrame) {
                ...
                if (frame.handle !== null) {
                    if (this._linksByRemoteHandle[frame.handle]) {
                        debug('Update link flow with ' + frame.linkCredit);
                    } else {
                        // @todo Proper error reporting.  Should we shut down session?
                        console.warn('Received Flow for unknown link ' + frame.handle + ': ' + JSON.stringify(frame));
                    }
                } else {
                    debug('Update session flow with (IN=' + frame.incomingWindow + ', OUT=' + frame.outgoingWindow+')');
                }
            } else {
...
Session.prototype._beginReceived = function(frame) {
    ...
    this.sessionParams.handleMax = this.sessionParams.handleMax ?
        Math.min(this.sessionParams.handleMax, frame.handleMax || constants.defaultHandleMax)
        : (frame.handleMax || constants.defaultHandleMax);
    debug('On BEGIN_RCVD, setting params to ('+this.sessionParams.nextIncomingId+','+this.sessionParams.remoteIncomingWindow+','+
        this.sessionParams.remoteOutgoingWindow+','+this.sessionParams.handleMax+')');
...
Link.prototype.attachReceived = function(attachFrame) {
    ...
    debugLink('Rx attach CH='+attachFrame.channel+', Handle='+attachFrame.handle);
