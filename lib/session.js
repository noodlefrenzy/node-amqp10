...
Session.prototype.attachLink = function(linkPolicy) {
    var linkParams = u.deepMerge(linkPolicy.options);
    if (typeof linkParams.name === 'function') linkParams.name = linkParams.name();
    linkParams.handle = this._nextHandle();
    var newLink = new Link(this, linkParams.handle, linkPolicy);
    this._allocatedHandles[linkParams.handle] = newLink;
...
Link.prototype.addCredits = function(credits, flowOptions) {
    ...
    var opts = flowOptions || {};
    opts.linkCredit = this.linkCredit + credits;
    this.linkCredit = opts.linkCredit;
...
Link.prototype.messageReceived = function(transferFrame) {
    this.linkCredit--;
    debugLink('Rx message ' + transferFrame.deliveryId + ' on ' + this.name);
...
Link.prototype.sendMessage = function(message, options) {
    this.session.sendMessage(this, message, options);
};
...
Link.prototype._detached = function() {
    ...
    this.session._allocatedHandles[this.linkParams.handle] = undefined;
    this.attached = false;
