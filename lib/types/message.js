...
function defineCompositeSection(definition) {
  var sectionDefinition = {
    ...
    encode: function(value, buffer) {
      ...
      for (var i = 0; i < _len; ++i) {
        ...
        if (value && value.hasOwnProperty(field.name)) {
          result[i] = field.type !== '*' ? new ForcedType(field.type, value[field.name]) : value[field.name];
        } else if (field.hasOwnProperty('default')) {
          result[i] = field.type !== '*' ? new ForcedType(field.type, field.default) : field.default;
        } else {
          result[i] = null;
        }
      }
  ...
  var symbolicDescriptor = 'amqp:' + definition.name + ':list';
  sectionByDescriptor[definition.code] = sectionDefinition;
  sectionByDescriptor[symbolicDescriptor] = sectionDefinition;
...
function defineMapSection(definition) {
  ...
  var symbolicDescriptor = 'amqp:' + definition.name + ':map';
  sectionByDescriptor[definition.code] = sectionDefinition;
  sectionByDescriptor[symbolicDescriptor] = sectionDefinition;
