...
Codec.prototype._readFullValue = function(buf, offset, doNotConsume, forcedCode) {
  // if (offset === 0)
  //   console.log('_readFullValue: ', buf.toString('hex'), offset, doNotConsume, forcedCode);
...
Codec.prototype.encode = function(value, buffer, forceType) {
  ...
  if (value === null) {
    return types.builders.null(value, buffer);
  }
  ...
  switch (typeof value) {
    case 'boolean': return types.builders.boolean(value, buffer);
    case 'string': return types.builders.string(value, buffer);
    ...
    case 'number':
      ...
      if (!encoder) throw new errors.NotImplementedError('encode(' + typeName + ')');
      return encoder(value, buffer);
...
Codec.prototype._encodeObject = function(value, buffer) {
  if (value instanceof DescribedType) {
    ...
    this.encode(value.descriptor, buffer);
    this.encode(value.getValue() || [], buffer);
    return;
  } else if (value.encodeOrdering && value.encodeOrdering instanceof Array) {
  ...
  if (value instanceof Buffer) return types.builders.binary(value, buffer, this);
  if (value instanceof AMQPArray) return types.builders.array(value, buffer, this);
  if (value instanceof Array) return types.builders.list(value, buffer, this);
  if (value instanceof Date) return types.builders.timestamp(new Int64(value.getTime()), buffer, this);
  if (value instanceof ForcedType) return this.encode(value.value, buffer, value.typeName);
  if (value instanceof AMQPSymbol) return types.builders.symbol(value.contents, buffer, this);

  var encoder;
  if (value instanceof Int64) {
    encoder = (value < 0) ? types.builders.long: types.builders.ulong;
    return encoder(value, buffer, this);
  }  else if (value.encode && (typeof value.encode === 'function')) {
    return value.encode(this, buffer);
  }

  return types.builders.map(value, buffer, this);
};
