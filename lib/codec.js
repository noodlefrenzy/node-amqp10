...
var Codec = function() {
    ...
    this.variableParser = '%d:8, len:8, value:len/%s';
    ...
    for (var tName in constants.types) {
        ...
        var parserStr;
        if (t.variable) {
            parserStr = util.format(this.variableParser + this.decodeSuffix, t.code, t.type || 'binary');
        } else if (t.length) {
        ...
        debug(parserStr);
        var curParser = { parser: bitsyntax.parse(parserStr), builder: t.builder, name: tName.substring(1) };
...
Codec.prototype.encode = function(val, buf, offset) {
    ...
    switch (type) {
        case 'string':
            // TODO: Long strings (0xB1)
            var stringParser = util.format(this.variableParser, constants.types._utf8.code, 'binary');
