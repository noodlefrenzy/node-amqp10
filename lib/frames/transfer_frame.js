var debug       = require('debug')('amqp10-transfer_frame'),
    util        = require('util'),
...
function TransferFrame(options) {
    ...
    if (options instanceof DescribedType) {
        ...
        this.more = options.more || false;
        this.receiverSettleMode = options.receiverSettleMode;
...
TransferFrame.prototype._getPerformative = function() {
    ...
    return new DescribedType(TransferFrame.Descriptor.code, {
        ...
        deliveryId: new ForcedType('uint', u.orNull(self.deliveryId)),
        deliveryTag: u.orNull(self.deliveryTag),
        messageFormat: new ForcedType('uint', u.orNull(self.messageFormat)),
        settled: u.orNull(self.settled),
        more: u.onUndef(self.more, false),
        receiverSettleMode: new ForcedType('ubyte', u.orNull(self.receiverSettleMode)),
        state: u.orNull(self.state),
        resume: u.onUndef(self.resume, false),
        aborted: u.onUndef(self.aborted, false),
        batchable: u.onUndef(self.batchable, false),
...
TransferFrame.prototype.readPerformative = function(describedType) {
    ...
    this.deliveryId = input[idx++];
    this.deliveryTag = input[idx++] || null;
    this.messageFormat = input[idx++];
    this.settled = input[idx++];
    this.more = input[idx++] || false;
    this.receiverSettleMode = input[idx++];
    this.state = input[idx++];
    this.resume = input[idx++] || false;
    this.aborted = input[idx++] || false;
    this.batchable = input[idx++] || false;
};
