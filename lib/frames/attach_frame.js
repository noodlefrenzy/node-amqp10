var debug       = require('debug')('amqp10-attach_frame'),
    util        = require('util'),
...
function AttachFrame(options) {
    ...
    if (options instanceof DescribedType) {
        ...
        this.senderSettleMode = options.senderSettleMode || constants.senderSettleMode.mixed;
        this.receiverSettleMode = options.receiverSettleMode || constants.receiverSettleMode.autoSettle;
        this.source = options.source || null;
        this.target = options.target || null;
        this.unsettled = options.unsettled || {};
        this.incompleteUnsettled = options.incompleteUnsettled || false;
        this.initialDeliveryCount = options.initialDeliveryCount || null;
...
AttachFrame.prototype._getPerformative = function() {
    var self = this;
    return new DescribedType(AttachFrame.Descriptor.code, {
        name: self.name,
        ...
        source: self._sourceAsPerformative(self.source),
        target: self._targetAsPerformative(self.target),
        ...
        offeredCapabilities: self.offeredCapabilities || null,
        desiredCapabilities: self.desiredCapabilities || null,
...
AttachFrame.prototype._sourceAsPerformative = function(source) {
    if (!source) return null; // Possible bug - might need to be empty list instead of null.
    return new DescribedType(constants.describedTypes.source.code, {
        address: source.address,
        durable: new ForcedType('uint', source.durable === undefined ? constants.terminusDurability.none : source.durable),
        expiryPolicy: source.expiryPolicy === undefined ? constants.terminusExpiryPolicy.sessionEnd : source.expiryPolicy,
        timeout: new ForcedType('uint', source.timeout || 0),
        dynamic: source.dynamic || false,
        dynamicNodeProperties: source.dynamicNodeProperties || {},
        distributionMode: source.distributionMode || null,
        filter: source.filter || {},
        defaultOutcome: source.defaultOutcome || null,
...
AttachFrame.prototype._targetAsPerformative = function(target) {
    if (!target) return null;
    return new DescribedType(constants.describedTypes.target.code, {
        address: target.address,
        durable: new ForcedType('uint', target.durable === undefined ? constants.terminusDurability.none : target.durable),
        expiryPolicy: target.expiryPolicy === undefined ? constants.terminusExpiryPolicy.sessionEnd : target.expiryPolicy,
        timeout: new ForcedType('uint', target.timeout || 0),
        dynamic: target.dynamic || false,
        dynamicNodeProperties: target.dynamicNodeProperties || {},
...
AttachFrame.prototype.readPerformative = function(describedType) {
    ...
    var idx = 0;
    this.name = input[idx++];
    this.handle = input[idx++];
    this.role = input[idx++];
    this.senderSettleMode = input[idx++] || constants.senderSettleMode.mixed;
    this.receiverSettleMode = input[idx++] || constants.receiverSettleMode.autoSettle;
    this.source = this._readSource(input[idx++]);
    this.target = this._readTarget(input[idx++]);
    this.unsettled = input[idx++] || {};
    this.incompleteUnsettled = input[idx++] || false;
    this.initialDeliveryCount = input[idx++];
    this.maxMessageSize = input[idx++] || 0;
    this.offeredCapabilities = input[idx++] || [];
    this.desiredCapabilities = input[idx++] || [];
    this.properties = input[idx++] || {};
};

AttachFrame.prototype._readSource = function(sourceArr) {
    if (!sourceArr) return null;
    var idx = 0;
    return {
        address : sourceArr[idx++] || null,
        durable: sourceArr[idx++] || constants.terminusDurability.none,
        expiryPolicy: sourceArr[idx++] || constants.terminusExpiryPolicy.sessionEnd,
        timeout: sourceArr[idx++] || 0,
        dynamic: sourceArr[idx++] || false,
        dynamicNodeProperties: sourceArr[idx++] || {},
        distributionMode: sourceArr[idx++] || null,
        filter: sourceArr[idx++] || {},
        defaultOutcome: sourceArr[idx++] || null,
        outcomes: sourceArr[idx++] || [],
        capabilities: sourceArr[idx++] || []
    };
};

AttachFrame.prototype._readTarget = function(targetArr) {
