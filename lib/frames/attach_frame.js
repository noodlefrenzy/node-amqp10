...
AttachFrame.prototype._getPerformative = function() {
    ...
    return new DescribedType(AttachFrame.Descriptor.code, {
        name: self.name,
        handle: ForcedType('uint', self.handle),
        role: self.role,
        senderSettleMode: ForcedType('ubyte', self.senderSettleMode),
        receiverSettleMode: ForcedType('ubyte', self.receiverSettleMode),
        ...
        initialDeliveryCount: ForcedType('uint', self.initialDeliveryCount),
        maxMessageSize: ForcedType('ulong', self.maxMessageSize),
...
AttachFrame.prototype.readPerformative = function(describedType) {
    ...
    this.role = input[idx++];
    this.senderSettleMode = input[idx++];
    this.receiverSettleMode = input[idx++];
    this.source = input[idx++];
    this.target = input[idx++];
    this.unsettled = input[idx++];
    this.incompleteUnsettled = input[idx++];
    this.initialDeliveryCount = input[idx++];
    this.maxMessageSize = input[idx++];
...
AttachFrame.prototype._sourceAsPerformative = function(source) {
    if (!source) return null; // Possble bug - might need to be empty list instead of null.
    return new DescribedType(constants.describedTypes.source.code, {
        address: source.address,
        durable: ForcedType('uint', source.durable === undefined ? constants.terminusDurability.none : source.durable),
        expiryPolicy: source.expiryPolicy === undefined ? constants.terminusExpiryPolicy.sessionEnd : source.expiryPolicy,
        timeout: ForcedType('uint', source.timeout || 0),
...
AttachFrame.prototype._targetAsPerformative = function(target) {
    ...
    return new DescribedType(constants.describedTypes.target.code, {
        ...
        durable: ForcedType('uint', target.durable === undefined ? constants.terminusDurability.none : target.durable),
        expiryPolicy: target.expiryPolicy === undefined ? constants.terminusExpiryPolicy.none : target.expiryPolicy,
        timeout: ForcedType('uint', target.timeout || 0),
