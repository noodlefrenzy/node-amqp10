...
function compositeAccessors(index, field) {
  return {
    ...
    get: function() {
      if (this.value[index] === undefined && field.hasOwnProperty('default')) return field.default;
      return field.type === '*' ? this.value[index] : this.value[index].value;
    },
    set: function(value) {
      this.value[index] = field.type === '*' ? value : new ForcedType(field.type, value);
    }
  };
}

types.defineComposite = function(definition) {
  ...
  for (var i = 0; i < definition.fields.length; i++) {
    ...
    Object.defineProperty(Composite.prototype, field.name, compositeAccessors(i, field));
  }
  ...
  Composite.create = function(fields) {
    var result = new Composite();
    var _keys = Object.keys(fields), _len = _keys.length;
    for (var i = 0; i < _len; ++i) { result[_keys[i]] = fields[_keys[i]]; }
    return result;
  };

  Composite.prototype.toDescribedType = function(fields) {
    return new DescribedType(Composite.descriptor.code, new ForcedType('list', this.value));
  };

  Composite.prototype.fromDescribedType = function(described) {
    // var _len = described.value.length;
    // for (var i = 0; i < _len; ++i) { this}
  };
