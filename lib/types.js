...
function arrayBuilder(val, bufb, codec, width) {
  ...
  var encode = types.builders[val.elementType];
  if (!encode) {
  ...
  for (var _i = 0; _i < _len; ++_i) encode(val.array[_i], tempBufb, codec);
...
function arrayDecoder(width, buffer, codec) {
  ...
  var decoder = types.decoders[elementType];
  if (!decoder) {
...
function registerType(name, options) {
  if (!!options.builder) {
    types.builders[name] = options.builder;
  } else {
    // default to first encoding
    var typeCode = options.encodings[0].code;
    types.builders[name] = function(val, bufb) {
      bufb.appendUInt8(typeCode);
      types.builders[typeCode](val, bufb);
    };
  }

  options.encodings.forEach(function(encoding) {
    types.decoders[encoding.code] = encoding.decoder;
    types.builders[encoding.code] = encoding.builder;
  });
}
...
registerType('long', {
  builder: function(val, bufb) {
    ...
    bufb.appendUInt8(code);
    types.builders[code](val, bufb); // @todo Deal with Math.abs(val) < 0x7F cases
  },
  encodings: [{
    code: 0x81,
    builder: function(val, bufb) {
...
registerType('string', {
  builder: function(val, bufb) {
    ...
    bufb.appendUInt8(code);
    types.builders[code](encoded, bufb);
  },
  encodings: [{
    code: 0xa1,
    builder: function(val, bufb) {
...
registerType('array', {
  builder: function(val, bufb, codec) { arrayBuilder(val, bufb, codec); },
  encodings: [{
    code: 0xe0,
