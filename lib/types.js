...
Types.prototype.getCode = function(val, forcedType) {
    if (val === null) {
        return 0x40;
    } else if (forcedType) {
        // They gave us the actual code they want.
        if (typeof forcedType === 'number') return forcedType;
    }
    ...
    if (valType === 'string') {
        ...
        if (val % 1 === 0) {
            ...
            if (val < 0) {
                // Signed type
                absval = Math.abs(val);
                if (absval > 0x7FFFFFFF) {
                    amqpTypeName = 'long';
                    width = 8;
                } else if (absval > 0x7FFF) {
                    amqpTypeName = 'int';
                    width = 4;
                } else if (absval > 0x7F) {
                    amqpTypeName = 'short';
            ...
            // Float type - for now, force double
            absval = Math.abs(val);
            if (absval > 3.402823E+38) {
                amqpTypeName = 'double';
                width = 8;
            } else {
                amqpTypeName = 'float';
        ...
        if (val instanceof Int64) {
            return 0x81; // Int64 => longest long for now
        } else if (val instanceof Array) {
            if (val.length === 0) return 0x40; // Empty arrays === null
            var types = val.map(function (x) { return typeof x; });
            // Differing types == lists.  @todo Replace with .find when ECMAScript6 comes out.
            var differs = false;
            types.forEach(function (x) { if (x !== types[0]) differs = true; });
            if (differs) return 0xD0;
            // Strings and numbers can always up-convert to longer string/number type.
            if (types[0] === 'string' || types[0] === 'number') return 0xF0;

            // For non-string/numbers, we're more picky.
            // We could be too picky here (e.g. ForcedType of symbol with short and long ones), but oh well (v2?).
            var arrCode;
            for (var idx in val) {
                var curVal = val[idx];
                var curCode = this.getCode(curVal);
                if (arrCode && arrCode != curCode) return 0xD0; // Assume "long" lists.
                arrCode = curCode;
            }
            return 0xF0; // Assume "long" arrays.
        } else {
            // Assume "map" unless encodeOrdering
            if (val.encodeOrdering && val.encodeOrdering instanceof Array) return 0xD0; // Assume "long" lists.
            return 0xD1; // Assume "long" maps.
        }
    }

    var amqpType = this.typesByName[forcedType || amqpTypeName];
    if (!amqpType) {
        throw new exceptions.NotImplementedError('Unable to find type for ' + (amqpTypeName || valType));
    }

    // If there's only one way to encode the value, return that code.
    if (amqpType.encodings.length === 1) return amqpType.encodings[0].code;

    switch (amqpType.name) {
        case 'string':
            // Fudge the numbers a bit here, since UTF values might encode to longer, so err on the side of "long strings"
            return (val.length > 0x7F) ? 0xB1 : 0xA1;
        case 'symbol':
            // No such constraint here, since symbols are ASCII
            return (val.length > 0xFF) ? 0xB3 : 0xA3;
        case 'boolean':
            return val ? 0x41 : 0x42;
        case 'binary':
            return (val.length > 0xFF) ? 0xB0 : 0xA0;
        case 'uint':
        case 'ulong':
        case 'int':
        case 'long':
            var bestEncoding;
            var delta;
            for (var encIdx in amqpType.encodings) {
                var curEncoding = amqpType.encodings[encIdx];
                if (curEncoding.width >= width) {
                    var curDelta = curEncoding.width - width;
                    if (!bestEncoding || curDelta < delta) {
                        bestEncoding = curEncoding;
                        delta = curDelta;
                    }
                }
            }
            return bestEncoding.code;

        default:
            throw new exceptions.NotImplementedError('Type ' + amqpType.name + ' has more than one encoding and is not explicitly dealt with.');
    }
};
