var debug       = require('debug')('amqp10-types'),
    ...
    AMQPArray   = require('./types/amqp_array'),
        ...
        if (val instanceof AMQPArray) {
            if (!width && val.array.length === 0) {
                ...
                if (width === 1 || (width !== 4 && arrayBytes.length < 0xFF && val.array.length < 0xFF)) {
                    ...
                    bufb.appendUInt8(arrayBytes.length+1);
                    bufb.appendUInt8(val.array.length);
                } else {
                    if (!width) bufb.appendUInt8(0xF0);
                    bufb.appendUInt32BE(arrayBytes.length+4);
...
Types.prototype._arrayDecoder = function(countSize, buf, codec) {
    ...
    for (var idx=0; idx < count; ++idx) {
        ...
        if (!decoded) {
            throw new exception.MalformedPayloadError('Unable to decode value of '+elementType.toString(16)+' from buffer '+buf.toString('hex'));
        }
        result.push(decoded[0]);
        offset += decoded[1];
    }
        ...
        },
        {
            ...
            "encodings": [
                ...
                },
                {
                    ...
                    "builder": function(val, bufb) { bufb.appendUInt8(val); },
                    "decoder": function(buf) { return butils.readInt(buf, 0); }
                },
                {
                    "code": "0x44",
                    "category": "fixed",
