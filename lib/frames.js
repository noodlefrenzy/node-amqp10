...
function defineFrame(type, definition) {
  ...
  // @todo: this guy is pretty complex
    ...
    for (var i = 0; i < _len; ++i) {
      ...
      if (this.hasOwnProperty(field.name)) {
        if (definition.fields[i].type === '*') {
          value.push(this[field.name]);
        } else if (field.multiple === true) {
          if (Array.isArray(this[field.name])) {
            if (this[field.name].length === 0) {
              value.push(null);
            } else if (this[field.name].length === 1) {
              value.push(new ForcedType(field.type, this[field.name][0]));
            } else {
              var values = [], _len2 = this[field.name].length;
              for (var j = 0; j < _len2; ++j) {
                values.push(new ForcedType(field.type, this[field.name][j]));
              }

              value.push(new ForcedType('list', values));
            }
          } else {
            value.push(new ForcedType(field.type, this[field.name]));
          }
        } else {
          value.push(new ForcedType(field.type, this[field.name]));
        }
