...
var Builder = require('buffer-builder'),
    ...
    SaslFrame = require('./frames/sasl_frame').SaslFrame,
    ...
    c = require('./constants'),
    types = require('./new_types'),
    terminus = require('./new_types/terminus');
...
frames.writeFrame = function(frame, stream) {
  ...
  if (performative !== null && performative !== undefined) {
    ...
    if (frame instanceof TransferFrame || frame instanceof frames.TransferFrame) {
  ...
  debug('sending frame: ' + frame.constructor.name + ': ' + buffer.toString('hex'));
  stream.write(buffer);
};
...
frames.readFrame = function(buffer) {
  ...
  // @todo: temporary
  var frame;
  if (described.descriptor === 0x14) {
    frame = new TransferFrame(described);
    frame.message = messageBuffer;
  } else {
    frame =  new framesByDescriptor[described.descriptor](described);
  }
...
function defineFrame(type, definition) {
  var FrameBase = (type === FrameType.AMQP) ? AMQPFrame : SaslFrame;
  var Frame = types.defineComposite(FrameBase, definition);
...
frames.HeartbeatFrame = function() {
  AMQPFrame.call(this, 0);
};
frames.HeartbeatFrame.prototype = Object.create(AMQPFrame.prototype);
